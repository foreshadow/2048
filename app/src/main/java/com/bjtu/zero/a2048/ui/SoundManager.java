...
public class SoundManager  extends AsyncTask<java.lang.Void,java.lang.Void,java.lang.Void> {
    ...
    protected Void doInBackground(Void... voids) {
        ...
        mergenum[0][2] = dotaSoundPool.load(context, R.raw.dotadoublekill, 1);
        mergenum[0][3] = dotaSoundPool.load(context, R.raw.dotatriplekill, 1);
        mergenum[0][4] = dotaSoundPool.load(context, R.raw.dotaultrakill, 1);
        mergenum[0][5] = dotaSoundPool.load(context, R.raw.dotarampage, 1);
        ...
        mergenum[1][2] = lolSoundPool.load(context, R.raw.loldoublekill, 1);
        mergenum[1][3] = lolSoundPool.load(context, R.raw.loltriplekill, 1);
        mergenum[1][4] = lolSoundPool.load(context, R.raw.lolquatrekill, 1);
        mergenum[1][5] = lolSoundPool.load(context, R.raw.lolpentakill, 1);
        //load happy
            ...
            switch (Setting.Sound.SOUND_PACK) {
                ...
                default:
                    if (maxRank >= 2 && isFirstBlood) {
                        soundPool.play(firstblood[Setting.Sound.SOUND_PACK], 1, 1, 0, 0, 1);
                        isFirstBlood = false;
                    }
                    if (maxRank >= 7)
                        soundPool.play(rank[Setting.Sound.SOUND_PACK][Math.min(maxRank, 11)], 1, 1, 0, 0, 1);
                    else if (mergeNum >= 2&& System.currentTimeMillis()-lastDoubleKillTime > 2000){
                        soundPool.play(mergenum[Setting.Sound.SOUND_PACK][Math.min(mergeNum, 5)], 1, 1, 0, 0, 1);
